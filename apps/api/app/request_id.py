...
async def request_id_exception_handler(
    request: Request, _error: Exception
) -> PlainTextResponse:
    """Preserve Starlette's generic 500 response while echoing the request id."""
    request_id = request.scope.get("state", {}).get(_STATE_KEY)
    headers = {HEADER: request_id} if isinstance(request_id, str) else None
