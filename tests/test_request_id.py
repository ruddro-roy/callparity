...
def test_cors_preflight_echoes_and_logs_request_id(client, capsys):
    res = client.options(
        "/readyz",
        headers={
...
def test_access_line_redacts_phone_shaped_path(client, capsys):
    res = client.get(f"/missing/{SAMPLE}", headers={"X-Request-ID": KNOWN})

    line = _access(capsys)
    assert res.status_code == 404
    assert SAMPLE not in json.dumps(line)
    assert line["path"] == "/missing/[phone]"
...
def test_unhandled_500_echoes_request_id():
    isolated_app = FastAPI()
    isolated_app.add_exception_handler(Exception, request_id_exception_handler)
    ...
    with TestClient(isolated_app, raise_server_exceptions=False) as isolated_client:
        res = isolated_client.get("/boom", headers={"X-Request-ID": KNOWN})
